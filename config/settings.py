...
3. output_products: 该环节的关键产出（如产品、服务、中间件）。返回列表。
   - **注意**：对于下游环节，请重点关注具体的应用产品、服务对象或终端市场（例如生物医药领域的特定药物、诊疗服务、医院等）。
...
【示例参考】
- **粒度控制**：不要使用过于泛化的词（如“设备”、“材料”），而要使用具体名称（如“单晶炉”、“光伏玻璃”）。
- **错误示例**：input_elements: ["材料"]
- **正确示例**：input_elements: ["多晶硅", "石英坩埚"]
...
GLOBAL_CONTEXT_PROMPT = """你是一个资深的产业分析专家。请根据以下提供的文档片段，为“{user_topic}”行业生成一份【全局产业上下文摘要】。

这一摘要将用于辅助后续的详细信息抽取，因此需要明确指出当前文档数据库所覆盖的主要方向、粒度层级以及关键的分类体系。
...
任务要求：
1. **产业概览**：简要概括该产业的主要构成部分（上游、中游、下游）。
2. **主要分类**：列出文档中频繁出现的核心子领域或关键环节名称。
3. **数据粒度**：观察文档中的实体描述是偏向宏观（如“制造业”）还是微观（如“光刻机”、“单克隆抗体”）。请在摘要中说明，并给出建议的提取粒度。
4. **特定领域关注点**（如果适用）：例如对于生物医药，指出文档是否包含具体的药物名称、靶点或疾病领域；对于制造业，指出是否包含具体的设备型号或材料名称。

请输出一段清晰、简洁的摘要文本（500字以内）。
"""

# --- Graph Refinement Prompt ---
GRAPH_REFINEMENT_PROMPT = """你是一个严谨的数据治理专家。你的任务是审查并优化一份从非结构化文本中抽取出来的“产业链图谱数据”。
由于数据是分批抽取的，可能存在以下问题：
1. **同名异义/同义异名**：例如 "Bio-reactor" 和 "Bioreactor" 实际上是同一个东西。
2. **粒度不统一**：有的节点是 "反应器"，有的是 "不锈钢生物反应器"。
3. **层级混乱**：某些节点可能实际上是另一个节点的子类。

当前提取的节点列表：
{nodes_list}

请根据节点名称的相似性和逻辑关系，建议合并或重命名的操作。
请以 JSON 格式返回优化建议：
{{
  "merge_suggestions": [
    {{
      "source_nodes": ["Bioreactor", "Bio-reactor", "生物反应器"],
      "target_node": "生物反应器",
      "reason": "同义词合并"
    }},
    {{
      "source_nodes": ["Apple", "Apples"],
      "target_node": "Apple",
      "reason": "单复数归一化"
    }}
  ]
